...
One important character of tweets is that they contain a lot of emojis, in the form of `latin1` characters, which would hinder text analysis. To eliminate that problem, I convert all characters in the form of `latin1` to `ASCII`.
...
To calculate word frequency and make a wordcloud out of the tweets, I use the `tm` package. First, I create a [text corpus](https://en.wikipedia.org/wiki/Text_corpus) by using the `Corpus` function in the package:

```r
library(tm) #text mining
brexit.corp = Corpus(VectorSource(orig$text))
```

Although the data have been cleaned and ready to analyze as individual tweets, the corpus needs to be cleaned again to remove common word endings, white spaces, [stop words](https://en.wikipedia.org/wiki/Stop_words), punctuations, and the likes. 
