...
Not so surprising, the top 10 retweeted tweets all come from a famous source, in this case, [Edward Snowden](https://twitter.com/Snowden). With the same trick, I can check the polarity scores of the tweets:
...
```r
pol.vs.retweet = ggplot(brexit, aes(x = pol, y = retweetCount)) +
  geom_point(position = 'jitter') +
  geom_smooth()
pol.vs.retweet + ggtitle("Polarity vs Retweet") + xlab('Polarity') + ylab("Number of Retweets")
```
